    ...
    <section className="py-16 px-6 md:px-16 lg:px-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-14 mt-14">
        {locationData.map((section, index) => (
          <div key={index}>
            {/* Title */}
            <h3 className="text-[#062E2C] text-base md:text-lg lg:text-xl font-[700] font-raleway uppercase mb-7">
              {section.title}
            </h3>
            ...
            <div className="space-y-4">
              {section.places.map((place, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between gap-4 text-[#062E2C] font[400] text-sm md:text-lg lg:text-lg"
                >
